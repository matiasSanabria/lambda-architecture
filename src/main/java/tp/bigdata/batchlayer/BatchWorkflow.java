...
/**	public static void initTestData() throws Exception {
        ...
        Pail<Data> masterPail = Pail.create(MASTER_ROOT, new SplitDataPailStructure());
        ...
        user1.set_name("User one");
        user1.set_birthday("31/03/1992");
        user1.set_address("En tu cora 1");
        user1.set_email("user1@gmail.com");
        user1.set_phone("0000000");
        // user2
        user2.set_user_id(2);
        user2.set_username("user2");
        user2.set_name("User two");
        user2.set_birthday("31/03/1992");
        user2.set_address("En tu cora 2");
        user2.set_email("user2@gmail.com");
        user2.set_phone("0000000");
        // product1
        p1.set_product_id(1);
        p1.set_barcode("01");
        p1.set_description("product one");
        p1.set_sale_price(2000);
        p1.set_stock(100);
        // product2
        p1.set_product_id(2);
        p1.set_barcode("02");
        p1.set_description("product two");
        p1.set_sale_price(4000);
        p1.set_stock(50);        
    ...
    public static class NormalizeURL extends CascalogFunction {
        public void operate(FlowProcess process, FunctionCall call) {
            ...
            DataUnit du = data.get_data_unit();
    ...
    public static class ExtractPageViewFields
        ...
        public void operate(FlowProcess process, FunctionCall call) {
            ...
            PageViewEdge pageview = data.get_data_unit()
                                        .get_page_view();
    ...
    public static class EdgifyEquiv extends CascalogFunction {
        public void operate(FlowProcess process, FunctionCall call) {
            Data data = (Data) call.getArguments().getObject(0);
            EquivEdge equiv = data.get_data_unit().get_equiv();
            call.getOutputCollector().add(
    ...
    public static class IterateEdges extends CascalogBuffer {
        public void operate(FlowProcess process, BufferCall call) {
            User grouped = (User) call.getGroup()
                                              .getObject(0);
            TreeSet<User> allIds = new TreeSet<User>();
            allIds.add(grouped);
            ...
            Iterator<User> allIdsIt = allIds.iterator();
            User smallest = allIdsIt.next();
            boolean isProgress = allIds.size() > 2 &&
                                 !grouped.equals(smallest);
            while(allIdsIt.hasNext()) {
                User id = allIdsIt.next();
                call.getOutputCollector().add(
    ...
    public static class MakeNormalizedPageview
        extends CascalogFunction {
        public void operate(FlowProcess process, FunctionCall call) {
            User newId = (User) call.getArguments()
                                            .getObject(0);
    ...
    public static Tap runUserNormalizationIteration(int i) {
    ...
    public static void normalizeUsers() throws IOException {
        Tap equivs = attributeTap("/tmp/swa/normalized_users",
        ...
        int i = 1;
        while(true) {
          Tap progressEdgesSink = runUserNormalizationIteration(i);
        ...
        Tap pageviews = attributeTap("/tmp/swa/normalized_users",
                DataUnit._Fields.USER_PROPERTY);
